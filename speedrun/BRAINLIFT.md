...
| **Application is constrained by retrievable knowledge — you can't apply what you can't retrieve** (Willingham; Bjork: retrieval strength decays with disuse) | **Performance is memory-gated**: each correct answer counts toward _current_ applied ability only in proportion to the concept's current FSRS retrievability (÷ desired retention), so Performance **declines with disuse** and recovers with practice — not just Memory. Readiness therefore drops no separate memory ceiling (avoids double-counting)              | `rslib/src/speedrun/concepts.rs` (`speedrun_topic_retention`), `scores.rs` (`performance_score` gate), `content.rs`, `models/performance.md`  |
