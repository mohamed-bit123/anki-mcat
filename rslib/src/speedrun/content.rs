...
impl Collection {
    ...
    /// syncs to every device. Wrapped in one undoable op.
    pub(crate) fn speedrun_record_calibration(
        ...
        self.transact(Op::UpdateConfig, |col| {
            let mut points: Vec<[f32; 2]> = col.get_config_default(CALIBRATION_CONFIG_KEY);
            ...
            col.set_config(CALIBRATION_CONFIG_KEY, &points)?;
            Ok(())
        })?;
        Ok(())
    }
    ...
    pub(crate) fn speedrun_set_exam_date(&mut self, timestamp: i64) -> Result<()> {
        self.transact(Op::UpdateConfig, |col| {
            col.set_config(EXAM_TIMESTAMP_CONFIG_KEY, &timestamp.max(0))?;
            Ok(())
        })?;
        Ok(())
    }
...
mod test {
    ...
    /// Builds a collection whose Readiness score is known (memory + full-depth
    /// performance across 3 topics), so calibration behaviour can be observed.
    fn collection_with_known_readiness() -> Collection {
        let mut col = Collection::new();
        for _ in 0..25 {
            add_flashcard(&mut col, DeckId(1), 3);
        }
    ...
    #[test]
    fn calibration_point_is_recorded_and_read_back() {
        let mut col = Collection::new();
        assert!(col.read_calibration().is_empty());
        col.speedrun_record_calibration(508.0, 511.0).unwrap();
        col.speedrun_record_calibration(512.0, 509.0).unwrap();
        let points = col.read_calibration();
        assert_eq!(points.len(), 2);
        assert_eq!(points[0].projected, 508.0);
        assert_eq!(points[1].actual, 509.0);
    }

    #[test]
    fn calibration_is_clamped_to_the_mcat_scale() {
        let mut col = Collection::new();
        // Values outside 472..=528 must be clamped, not stored raw.
        col.speedrun_record_calibration(999.0, 100.0).unwrap();
        let p = col.read_calibration()[0];
        assert_eq!(p.projected, 528.0);
        assert_eq!(p.actual, 472.0);
    }

    #[test]
    fn calibration_updates_readiness_note_and_is_undoable() {
        let mut col = collection_with_known_readiness();

        // Before calibration: readiness is known but flagged uncalibrated.
        ...
        // Record a real full-length outcome, then recompute.
        ...
        // A single undo reverts the calibration (config is captured by undo).
        col.undo().unwrap();
        let reverted = col.speedrun_compute_scores().unwrap();
        assert!(reverted
            .readiness
            .calibration_note
            .contains("Not yet calibrated"));
